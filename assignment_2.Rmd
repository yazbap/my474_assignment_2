...
- The utilization of momentum in optimization processes unavoidably increases memory requirements. This arises from the introduction of an additional term and hyperparameter that necessitate storage and ongoing updates. Despite these inherent constraints, leveraging additional memory presents opportunities for potentially optimizing our approach more effectively.
...
Weight Decay: Adding a weight decay term to the optimization objective can help regularize the optimization process and prevent excessive oscillations or overshooting.

Gradient Clipping: Limiting the magnitude of gradients during training can help stabilize the optimization process and prevent exploding gradients, which can be exacerbated by momentum. This is similar to the concept of LASSO or Rigde regression in which we limit the size coefficients may sum up to.

Monitoring and Analysis:

Monitoring Convergence: Regularly monitoring convergence metrics, such as training loss or validation accuracy, can help identify issues such as overshooting or stagnation early on. Adjustments to momentum-related parameters can then be made accordingly.
...
```{r}
y_pred <- predict(optimal_model, newx = as.matrix(cbind(0,0,0,x1)))
...
For this report, four models were under consideration, KNN, Trees, and LASSO and Ridge regression. KNN and Trees were found to not be suitable for predicting civil wars due to the high dimensionality of the model and their limitations in prediction accuracy, respectively. Consequently, the choice between Ridge and LASSO regression was more pertinent. In this work, LASSO regression was optend for in light of findings from Ward, Greenhill, and Bakke (2010), where they suggested that a parsimonious model, as facilitated by LASSO regression, often outperforms complex models in predicting civil war occurrences.
