...
Why might we want to predict civil war? According to Hegre et.al (2019), the more the UN is willing to spend on peacekeeping, the greater conflict reducing effect with these researchers finding that in the  most ambitious scenario modeled in 2013, the UN could have reduced major world conflicts by 70% with the appropriate peace keeping allocations. Therefore, where to spend these resources given where we think civil war may happen becomes an essential question. This training model attempts to predict to civil war to bettwe inform decision w=on where these resources should go. 

In class the models we learned so far are KNN, Trees and LASSO and Ridge regression. KNN would not be good in this case because the model has a high number of dimensions. Trees also would not work because they are rarely the best in terms of prediction. Therefore, in the choice between Ridge or LASSO regression I chose LASSO because as assessed by a paper by Ward, Greenhill and Bakke (2010) in which they state "when it comes to choosing a model that best predicts the occurrence of civil war, a very parsimonious model can often fare better than one that contains a relatively large number of statistically-significant variables."
